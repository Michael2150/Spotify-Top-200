...
Private Sub ChangeOffset(index As Int, dx As Int, complete As Boolean) As Boolean
	...
	Dim aItem As CLVItem = mCLV.GetRawListItem(index)
	If Not(aItem.Value Is SwipeItem) Then Return False
	Dim m As SwipeItem = aItem.Value
	...
	If m.IsSwiped = False Then
		If m.Actions.IsInitialized And m.Actions.Size > 0 Then
			...
			aItem.Panel.Parent.AddView(ActionsPanel, 0, aItem.Panel.Top, aItem.Panel.Width, aItem.Panel.Height)
		...
		End If
	End If
	Dim NewLeft As Int = Max(Min(aItem.Panel.Left + dx, 0), -m.MaxSwipe)
	If complete Then
		If (m.Open = False And NewLeft >= -40dip) Or (m.Open = True And NewLeft >= -m.MaxSwipe + 10dip) Then
			aItem.Panel.SetLayoutAnimated(200, 0, aItem.Panel.Top, aItem.Panel.Width, aItem.Panel.Height)
			m.IsSwiped = False
			m.Open = False
			ActionsPanel.RemoveViewFromParent
		Else 
			aItem.Panel.SetLayoutAnimated(200, -m.MaxSwipe, aItem.Panel.Top, aItem.Panel.Width, aItem.Panel.Height)
			m.Open = True
		End If
	Else

		aItem.Panel.Left = NewLeft
	End If
	Return True
End Sub
